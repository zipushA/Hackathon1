...
//   //שלב B7 — להפעיל את ההאזנה אוטומטית אחרי פתיחת שיחה
//   async function handleStartChat() {
//     if (!selectedScript) {
//       alert("בחרי תסריט");
//       return;
...
//           `data:audio/mp3;base64,${data.opening_audio_base64}`
//         );
...
//     if (isRecording || loading || isAssistantSpeakingRef.current) {
//       return;
//     }
...
//   //שלב B3 — להוסיף פונקציית זיהוי שקט
...
//   //       audio.onended = () => {
//   //         // פה אפשר להתחיל שוב הקלטה אוטומטית אם תרצי
//   //         // startRecording();
//   //       };
...
//   //שלב B6 — לשפר את sendRecordedAudio()
//   async function sendRecordedAudio(file: File) {
//     if (!sessionId) return;
...
//                     התחילי שיחה והמערכת תנהל הקשבה ודיבור בצורה אוטומטית.
//                   </p>
//                 </div>
//               </div>
...
//                           disabled={!sessionId || loading || isAssistantSpeakingRef.current}
//                           className="gap-2"
//                           size="sm"
//                         >
...
export default function ChatPage() {
  ...
  async function handleStartChat() {
    ...
    try {
      ...
      if (data.opening_audio_base64) {
        await playAssistantAudio(data.opening_audio_base64, newSessionId);
  ...
  async function handleSendText() {
    ...
    try {
      ...
      if (data.audio_base64) {
        await playAssistantAudio(data.audio_base64, sessionId);
      }
    } catch (error) {
      console.error(error);
      alert("שגיאה בשליחת הודעה");
  ...
  async function startRecording(customSessionId?: string) {
    ...
    if (isRecordingRef.current || loading || isAssistantSpeakingRef.current) {
      return;
    }
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        await sendRecordedAudio(audioFile, activeSessionId);
      };
  ...
  async function sendRecordedAudio(file: File, customSessionId?: string) {
    const activeSessionId = customSessionId || sessionId;
  ...
  return (
    <div className="h-screen overflow-hidden bg-slate-50" dir="rtl">
      <div className="mx-auto flex h-full max-w-7xl flex-col px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[1fr_280px]">
          <Card className="flex min-h-0 flex-col border-slate-200 shadow-sm">
            <CardHeader className="shrink-0 border-b border-slate-200 px-4 py-3">
              <div className="flex items-center justify-between">
                <CardTitle className="flex items-center gap-2 text-base">
                  <MessageCircle className="h-4 w-4 text-emerald-600" />
                  התכתבות
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {!isRecording ? (
                        <Button
                          ...
                          onClick={() => startRecording()}
                          disabled={
                            !sessionId || loading || isAssistantSpeakingRef.current
                          }
                          className="gap-2"
                          size="sm"
                        >
